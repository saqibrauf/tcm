...
def index(request):
	now = datetime.today()
	todays_matches = Match.objects.all().filter(date__day=now.day, date__month=now.month, date__year=now.year).order_by('date')
	...
	return render(request, 'match/index.html', context={'todays_matches':todays_matches, 'upcoming_matches':upcoming_matches, 'series': series, 'all_series':all_series})
